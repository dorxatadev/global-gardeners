...
const CLIMATE_OPTIONS: ClimateOption[] = [
  { id: "cold", title: "Cold", description: "Long winters, short growing season." },
  { id: "temperate", title: "Temperate", description: "Four seasons with moderate swings." },
  { id: "warm", title: "Warm", description: "Mild winters and long warm seasons." },
  { id: "tropical", title: "Tropical", description: "Hot, humid, and frost-free." },
];
...
function Radio({ checked }: { checked: boolean }) {
  return (
    <span
      aria-hidden="true"
      className={`inline-flex h-4 w-4 items-center justify-center rounded-full border ${checked ? "border-[#171717]" : "border-[#cfcfcf]"}`}
    >
...
export default function ClimatePage() {
  ...
  const canFinish = useMemo(() => Boolean(selectedId), [selectedId]);
  ...
  const handleFinish = async () => {
    ...
    router.push("/settings/experience/loading");
  };
    ...
    <main className="client-main min-h-screen bg-[radial-gradient(circle_at_top,_#fffdf7_0%,_#f8f6f1_50%,_#efe9dc_100%)] px-0 text-[#182a17] sm:grid sm:place-items-center sm:px-8">
      <section className="client-shell relative mx-auto flex min-h-screen w-full max-w-[390px] flex-col overflow-hidden border border-[#e7e0d2] bg-[#f8f6f1] shadow-[0_24px_80px_rgba(56,71,45,0.12)]">
        <header className="client-header fixed left-0 right-0 top-0 z-30 flex w-full items-center border-b border-black/10 bg-white p-4">
          <div className="flex items-center gap-3">
            <button
              ...
              onClick={() => router.push("/settings/experience/growing-zone")}
            >
              <BackIcon />
            </button>
        ...
        <div className="flex flex-1 min-h-0 flex-col px-4 pb-[112px] pt-[96px]">
          ...
          <h1 className="mt-3 text-[30px] font-semibold leading-[30px] tracking-[-1px] text-[#182a17]">What&apos;s your climate like?</h1>
          <p className="mt-4 text-[16px] font-medium leading-6 text-[#333333cc]">This helps us adjust recommendations to your environment.</p>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-[#f8f6f1] px-4 pb-4 pt-5">
          <button
            ...
            className={`flex h-[52px] w-full items-center justify-center rounded-full text-[14px] font-medium leading-5 text-[#fafafa] transition ${
              canFinish ? "bg-[#457941] hover:bg-[#3b6838]" : "cursor-not-allowed bg-[#99b297] pointer-events-none"
            }`}
            onClick={() => {
              if (!canFinish) return;
              void handleFinish();
